...
export async function upsertSavedSearch(userId, search) {
  const payload = { ...search, user_id: userId, updated_at: new Date().toISOString() };
...
export async function getJobs({ sort = 'score', limit = 20, offset = 0, userId, searchId } = {}) {
  ...
  if (sort === 'score') {
    query = query.order('score', { referencedTable: 'job_scores', ascending: false, nullsFirst: false });
...
export async function addFeedback(userId, jobId, feedback, note) {
  const { data, error } = await supabase
    .from('user_feedback')
    .upsert(
      { user_id: userId, job_id: jobId, feedback, note, created_at: new Date().toISOString() },
